...
import { GoogleAuthProvider, FacebookAuthProvider, signInWithPopup, signOut, createUserWithEmailAndPassword, signInWithEmailAndPassword, updateProfile } from 'firebase/auth';
...
export default function LoginModal({ isOpen, onClose, onLogin }: LoginModalProps) {
  ...
  const handleFacebookLogin = async () => {
    ...
    try {
      ...
      const result = await signInWithPopup(auth, provider);
      const userData = {
        name: result.user.displayName || 'ACQUIRER_FB',
        email: result.user.email || '',
        uid: result.user.uid
      };
      ...
      if (error.code === 'auth/operation-not-allowed') {
        setVerificationError('FACEBOOK_AUTH_DISABLED // ENABLE_IN_FIREBASE_CONSOLE');
      } else {
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <button 
                  ...
                  disabled={isLoading}
                  className="flex items-center justify-center gap-2 bg-bg border border-white/10 py-4 text-[9px] font-black uppercase tracking-widest hover:border-accent hover:text-accent transition-all group disabled:opacity-50"
                ...
                <button 
                  ...
                  disabled={isLoading}
                  className="flex items-center justify-center gap-2 bg-bg border border-white/10 py-4 text-[9px] font-black uppercase tracking-widest hover:border-[#1877F2] hover:text-[#1877F2] transition-all group disabled:opacity-50"
